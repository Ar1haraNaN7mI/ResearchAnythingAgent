...
function resolveExportRootDir(): string {
    return (
        process.env.DRAWIO_EXPORT_DIR?.trim() ||
        process.env.NEXT_AI_DRAWIO_OUTPUT_DIR?.trim() ||
        ""
    )
}

function readDiskPngBackgroundPref(): "transparent" | "white" | null {
    const root = resolveExportRootDir()
    if (!root) return null
    try {
        const p = path.join(root, ".png_background")
        const t = fs.readFileSync(p, "utf-8").trim().toLowerCase()
        if (t === "white" || t === "opaque") return "white"
        if (t === "transparent" || t === "clear") return "transparent"
    } catch {
        /* missing file */
    }
    return null
...
/** PNG: transparent vs white backdrop (draw.io embed export). */
function pngExportUseTransparent(toolBg?: "transparent" | "white"): boolean {
    if (toolBg === "white") return false
    if (toolBg === "transparent") return true
    const env = (
        process.env.DRAWIO_EXPORT_PNG_BACKGROUND || "transparent"
    ).toLowerCase()
    if (env === "white" || env === "opaque") return false
    if (env === "transparent" || env === "clear") return true
    const disk = readDiskPngBackgroundPref()
    if (disk === "white") return false
    return true
}
    ...
    "export_diagram",
    {
        ...
        inputSchema: {
            path: z
                ...
                .describe(
                    "File path to save the diagram (e.g., ./diagram.drawio, ./diagram.png, ./diagram.svg). " +
                        "If relative and DRAWIO_EXPORT_DIR is set, it is resolved under that directory.",
                ),
            format: z
                .enum(["drawio", "png", "svg"])
                .optional()
            ...
            background: z
                ...
                .optional()
                .describe(
                    "PNG only: transparent (default) or opaque white background. " +
                        "Overrides DRAWIO_EXPORT_PNG_BACKGROUND and drawio_exports/.png_background when set.",
                ),
        },
    },
    async ({ path: filePathInput, format, background }) => {
        try {
            if (!currentSession) {
                return {
                    content: [
            ...
            const exportRoot = resolveExportRootDir()
            let logicalPath = filePathInput
            if (exportRoot && !nodePath.isAbsolute(logicalPath)) {
                logicalPath = nodePath.join(exportRoot, logicalPath)
            }

            // Detect format from extension if not specified
            const ext = nodePath.extname(logicalPath).toLowerCase()
            ...
            if (detectedFormat === "drawio") {
                ...
                await fsp.mkdir(nodePath.dirname(absolutePath), {
                    recursive: true,
                })
                await fsp.writeFile(absolutePath, currentSession.xml, "utf-8")
            ...
            state.exportPngTransparent =
                detectedFormat === "png"
                    ? pngExportUseTransparent(background)
                    : undefined
            ...
            if (detectedFormat === "png") {
                ...
                await fsp.writeFile(absolutePath, svgContent, "utf-8")
            }
